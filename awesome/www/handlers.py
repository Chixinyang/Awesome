...
def wrapfunc(func): #装饰器打印函数名字
    def run(*args, **kw):
        logging.debug("go to func : {}".format(func.__name__))
        temp = func(*args, **kw)
        #logging.debug("back from func : {}".format(func.__name__))
        return temp
    return run
    
def check_admin(request):  #管理权限检查
    if request.__user__ is None or request.__user__.admin:
        raise APIPermissionError()


def get_page_index(page_str):  #获取页面
    p = 1
    try:
        p = int(page_str)
...
def user2cookie(user, max_age):#在服务端创建用户cookie
    '''
...
def text2html(text):#把文本转换成能够在html上显示文本
    #把不便于在http中传输特殊的字符用对应的字符实体替换                                                             过滤掉空行
    lines = map(lambda s: '<p>%s</p>' % s.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;'), filter(lambda s: s.strip() != '', text.split('\n')))
    return ''.join(lines)


@asyncio.coroutine 
def cookie2user(cookie_str):  #检查用户浏览器返回的cookie是否正确
    '''
...
def api_register_user(*, email, name, passwd):
    ...
    logging.debug("registing user email is :{}".format(email))
